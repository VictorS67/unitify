...
import {
  getEmissionFromDistance,
  getCaloriesFromDuration,
  getEmissionTrendIconByNumber,
} from "../Utils/TravalInfo";
...
const MapNav = (props) => {
  ...
  const [totalDuration, sTotalDuration] = useState(
    map.destination.duration.value
  );
  ...
  useEffect(() => {
    ...
    tripnav.travalDurations.forEach((travalDuration) => {
      ...
      actualEmission += getEmissionFromDistance(
        Number(travalDuration.distance) * 1000,
        travalDuration.travalMode
      );
      actualCalories += getCaloriesFromDuration(
        Number(travalDuration.duration),
        travalDuration.travalMode
      );
    });

    let finalDuration =
      totalDuration > actualDuration ? totalDuration : actualDuration;
    ...
    if (durationLength === 1) {
      ...
      progressChildrenTemp[durationLength - 1].steps +=
        finalDuration - durations.reduce((a, b) => a + b, 0);
    }

    sProgressChildren(progressChildrenTemp);

    const [hour, minute, second] = new Date(tripnav.startTimestamp)
      .toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        hour12: false,
      ...
      for (let index = 0; index < modes.length; index++) {
        if (modes[index] !== tripnav.currTravalMode) {
          ...
          if (testTravalMode === "BUS" || testTravalMode === "SUBWAY") {
            ...
            changeTravalModesTemp.push(
              renderChangeTravalMode(testTravalMode, nearBy, index)
            );
          } else {
            changeTravalModesTemp.push(
              renderChangeTravalMode(testTravalMode, true, index)
            );
          }
        }
      }
  ...
  const renderChangeTravalMode = (travalMode, isAvailable, key) => {
    return (
      <Pressable
        ...
        key={key}
        onPress={() => {
          changeTravalMode(travalMode, isAvailable);
        }}
      >
        {travalMode === "SUBWAY" && (
          <React.Fragment>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <Text
                style={{ color: theme.colors.text, fontSize: normalize(14) }}
              >
                Change Transit Type
              </Text>
            </TouchableOpacity>
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {calories >= 1000
                  ? `${Number((calories / 1000).toFixed(1))} kcal`
                  : `${calories} cal`}
              </Text>
            </View>
          </Text>
