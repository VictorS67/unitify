...
const UserChampionSignWindow = (props) => {
  ...
  const [maxWordCount, sMaxWordCount] = useState(
    50 - user.championSignature.length
  );
  ...
  const onPressChangeChampionSign = () => {
    if (chamSign !== "") {
      dispatch(changeChampionSignature(user.id, chamSign)).then((resolve) => {
        const result = JSON.parse(resolve);
        if (result.status === 200) {
          Alert.alert("Success", "You have changed your champion signature.", [
            {
              text: "OK",
    ...
    <View style={props.style}>
      ...
      >
        ...
        >
          <TextInput
            value={chamSign}
            onChangeText={(chamSignChange) =>
              onChangeTextChampionSign(chamSignChange)
            }
