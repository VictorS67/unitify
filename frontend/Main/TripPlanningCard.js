  ...
  useEffect(() => {
    try {
      ...
      ["DRIVING", "WALKING", "SUBWAY", "BUS", "BICYCLING"].forEach((mode) => {
        if (
          map.allDirection.hasOwnProperty(mode) &&
          map.allDirection[mode].destination.duration.value > largestDuration
        ) {
      ...
      let count = 1;
      let otherPlansTemp = Object.entries(map.allDirection).map(
        ([key, value]) => {
          count += 1;
  ...
  const getOtherPlanDetails = (
    ...
    return (
      <React.Fragment key={key}>
        ...
        <Pressable
          style={styles.optionsChoiceButton}
          onPress={() => {
            onTravalModePress(travalMode);
          }}
        >
          <View style={styles.optionsChoiceButtonText}>
            <Text style={styles.optionsChoiceText}>{travalMode}</Text>
            <Text style={styles.optionsChoiceText}>{durationText}</Text>
          </View>
          <ProgressBar
            progress={duration / largestDuration}
            color={option_colors[travalMode]}
          />
        </Pressable>
      </React.Fragment>
    );
        ...
        map.updateInfo === false && (
          <React.Fragment>
            ...
            <Pressable style={styles.button} onPress={onNavPress}>
              <Text style={styles.buttonTextLarge}>Let's Go!</Text>
            </Pressable>

            <View style={styles.optionsChoice}>
              <Text style={styles.optionsChoiceTitle}>Other Options</Text>
